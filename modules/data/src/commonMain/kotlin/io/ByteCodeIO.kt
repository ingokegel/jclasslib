...
fun writeByteCode(instructions: List<Instruction>): ByteArray {
    val output = BufferedDataOutput()
    for (instruction in instructions) {
        writeNextInstruction(output, instruction)
    }
    return output.toByteArray()
}

private fun writeNextInstruction(output: CountingDataOutput, instruction: Instruction) {
 ...
 * @param prependInstructions an array of instructions that is prepended. Can be null.
 * @return the list of instructions
 */
fun readByteCode(code: ByteArray, prependInstructions: Array<Instruction>? = null): ArrayList<Instruction> {

    val input = CountingSourceDataInput(code)
    ...
    var nextWide = false
    while (input.bytesRead < code.size) {
        val instruction = readNextInstruction(input, nextWide)
...
private fun readNextInstruction(input: CountingDataInput, wide: Boolean): Instruction =
    createInstruction(Opcode.getFromTag(input.readUnsignedByte()), wide).also { it.read(input) }
